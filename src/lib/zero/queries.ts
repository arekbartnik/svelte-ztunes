...
export const queries = {
	get_artists: syncedQuery('getHomepageArtists', z.tuple([z.string()]), (q: string) =>
		builder.artist.where('name', 'ILIKE', `%${q}%`).orderBy('popularity', 'desc').limit(20)
	),

	get_artist: syncedQuery('getArtist', z.tuple([z.string()]), (artistID: string) =>
		builder.artist
			.where('id', artistID)
	...
	get_cart_items: syncedQueryWithContext(
		'getCartItems',
		z.tuple([]),
